...
DirectWrite::DirectWrite()
{
	...
	assert(SUCCEEDED(D2D1CreateFactory(D2D1_FACTORY_TYPE_MULTI_THREADED,
		_factory.GetAddressOf())));
	...
	//assert(SUCCEEDED(Device::GetInstance()))
...
void DirectWrite::RenderText(wstring text, RECT rect, float fontSize, wstring font, XMFLOAT4 color, DWRITE_FONT_WEIGHT weight, DWRITE_FONT_STYLE style, DWRITE_FONT_STRETCH stretch)
{
	...
	FontBrushDesc* findText = nullptr;
	...
	if (findText == nullptr)
	{
		//assert(SUCCEEDED(_writeFactory->CreateTextFormat(textDesc._font.c_str,
			nullptr, textDesc._weight, textDesc._style, textDesc._stretch,
